...
* 对 [Claude Code](/zh-CN/docs/claude-code/quickstart) 的基本熟悉
...
在 [Agent Skills 概述](/zh-CN/docs/agents-and-tools/agent-skills/overview) 中了解更多。

<Note>
  有关 Agent Skills 的架构和实际应用的深入探讨，请阅读我们的工程博客：[使用 Agent Skills 为现实世界装备代理](https://www.anthropic.com/engineering/equipping-agents-for-the-real-world-with-agent-skills)。
</Note>
...
Skills 也可以来自 [Claude Code 插件](/zh-CN/docs/claude-code/plugins)。插件可能捆绑了在安装插件时自动可用的 Skills。这些 Skills 的工作方式与个人和项目 Skills 相同。
...
* `description`：Skill 功能及其使用时机的简要描述（最多 1024 个字符）

`description` 字段对于 Claude 发现何时使用您的 Skill 至关重要。它应该包括 Skill 的功能和 Claude 应该何时使用它。

查看 [最佳实践指南](/zh-CN/docs/agents-and-tools/agent-skills/best-practices) 以获取完整的创作指导，包括验证规则。
...
使用 `allowed-tools` 前置内容字段来限制当 Skill 处于活动状态时 Claude 可以使用哪些工具：
...
当此 Skill 处于活动状态时，Claude 只能使用指定的工具（Read、Grep、Glob），无需请求许可。这对以下情况很有用：

* 不应修改文件的只读 Skills
* 范围有限的 Skills（例如，仅数据分析，无文件写入）
* 安全敏感的工作流，您希望限制功能

如果未指定 `allowed-tools`，Claude 将按照标准权限模型要求许可来使用工具。
...
**推荐方法**：通过 [插件](/zh-CN/docs/claude-code/plugins) 分发 Skills。
...
有关完整说明，请参阅 [将 Skills 添加到您的插件](/zh-CN/docs/claude-code/plugins#add-skills-to-your-plugin)。
...
更改在您下次启动 Claude Code 时生效。如果 Claude Code 已在运行，请重启它以加载更新。
...
通过在描述中包含特定的触发词来帮助 Claude 发现何时使用 Skills：
...
模糊的描述会使发现变得困难。包括 Skill 的功能和使用时机，以及用户会提到的关键词。
...
Claude 将在需要时自动安装所需的依赖项（或要求许可安装它们）。
...
**在描述中要具体**：通过在描述中使用不同的触发词来帮助 Claude 选择正确的 Skill。
...
<CardGroup cols={2}>
  ...
  <Card title="Agent Skills 概述" icon="book" href="/zh-CN/docs/agents-and-tools/agent-skills/overview">
  ...
  <Card title="开始使用 Agent Skills" icon="rocket" href="/zh-CN/docs/agents-and-tools/agent-skills/quickstart">
    创建您的第一个 Skill
  </Card>
</CardGroup>
